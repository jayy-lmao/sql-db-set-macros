...
use crate::common::field_analysis::{
    FieldAnalysis, analyze_struct_fields, get_many_query_fields, 
    get_one_query_fields, get_insert_fields, get_key_fields_from_analysis,
    extract_field_names
};
use crate::common::utils::{get_table_name, get_dbset_name};
...
impl QueryBuilderConfig {
    ...
    pub fn new(input: &DeriveInput, query_type: QueryType) -> Result<Self, QueryBuilderError> {
        ...
        if struct_name.is_empty() {
            return Err(QueryBuilderError::InvalidConfig("Empty struct name".to_string()));
        }
        
        if table_name.is_empty() {
            return Err(QueryBuilderError::InvalidConfig("Empty table name".to_string()));
        }

        if dbset_name.is_empty() {
            return Err(QueryBuilderError::InvalidConfig("Empty dbset name".to_string()));
        }
        ...
        match self.query_type {
            ...
            QueryType::Update | QueryType::Delete => {
                ...
                if key_fields.is_empty() {
                    return Err(QueryBuilderError::InvalidConfig(
                        format!("{} query requires at least one key field", 
                               if matches!(self.query_type, QueryType::Update) { "Update" } else { "Delete" })
                    ));
                }
            }
            QueryType::Insert => {
...
pub trait QueryBuilderGenerator {
    ...
    fn filter_fields(&self, fields: &[FieldAnalysis]) -> Vec<FieldAnalysis> {
        match self.query_type() {
            ...
            QueryType::Delete => get_key_fields_from_analysis(fields).into_iter().cloned().collect(), // Only keys for deletes
        }
    }
    
    /// Validate configuration and fields before generation
    fn validate(&self, config: &QueryBuilderConfig, fields: &[FieldAnalysis]) -> Result<(), QueryBuilderError> {
        config.validate_fields(fields)?;
        let filtered_fields = self.filter_fields(fields);
        
        if filtered_fields.is_empty() {
            return Err(QueryBuilderError::FieldError(
                format!("No suitable fields found for {} query", config.query_type.description())
            ));
        }
    ...
    fn generate_sql_query(&self, config: &QueryBuilderConfig, fields: &[FieldAnalysis]) -> Result<String, QueryBuilderError> {
        match config.query_type {
            QueryType::Many => {
                ...
                let where_conditions = fields.iter().enumerate()
                    .map(|(i, field)| format!("({} = ${} OR ${} IS NULL)", field.name, i + 1, i + 1))
                    .collect::<Vec<_>>()
                    .join(" AND ");
            ...
            QueryType::Insert => {
                let field_names = extract_field_names(fields);
                let placeholders = (1..=fields.len()).map(|i| format!("${}", i)).collect::<Vec<_>>();
            ...
            QueryType::Update => {
                let key_fields = get_key_fields_from_analysis(fields);
                if key_fields.is_empty() {
                    return Err(QueryBuilderError::SqlError("Update requires key fields".to_string()));
                }
                
                let set_clauses = fields.iter().enumerate()
                    .filter(|(_, field)| !field.is_key)
                    .map(|(i, field)| format!("{} = ${}", field.name, i + 1))
                    .collect::<Vec<_>>()
                    .join(", ");
                
                let where_clauses = key_fields.iter().enumerate()
                    .map(|(i, field)| format!("{} = ${}", field.name, fields.len() - key_fields.len() + i + 1))
                    .collect::<Vec<_>>()
                    .join(" AND ");
            ...
            QueryType::Delete => {
                let where_clauses = fields.iter().enumerate()
                    .map(|(i, field)| format!("{} = ${}", field.name, i + 1))
                    .collect::<Vec<_>>()
                    .join(" AND ");
...
impl StandardQueryBuilder {
    fn generate_execute_method(&self, config: &QueryBuilderConfig, fields: &[FieldAnalysis]) -> Result<TokenStream, QueryBuilderError> {
        let struct_name = quote::format_ident!("{}", config.struct_name);
